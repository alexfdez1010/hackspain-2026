...
const DIRECTIONS: readonly string[] = ['improving', 'stable', 'deteriorating'];

const REGIMES: readonly string[] = [
  'steady',
  'structural_decline',
...
/**
 * Reads a dictionary of strings, used for the label maps.
 *
 * @param value - Candidate value.
...
/**
 * Reads a dictionary of numbers, used for alert counts and detection rates.
 *
 * @param value - Candidate value.
...
/**
 * Normalises the trajectory direction, defaulting to `stable`.
 *
 * @param value - Candidate value.
 * @returns A known direction.
 */
...
/**
 * Normalises the detected regime, defaulting to `steady`.
 *
 * @param value - Candidate value.
 * @returns A known regime.
 */
export function toRegime(value: unknown): Regime {
  return typeof value === 'string' && REGIMES.includes(value)
    ? (value as Regime)
    : 'steady';
}

/**
 * Normalises the six pillar scores, keeping `null` for missing evidence.
 *
 * @param value - Candidate value.
 * @returns A complete pillar map.
 */
export function toPillars(value: unknown): Pillars {
  const record = asRecord(value) ?? {};
  const pillars = {} as Pillars;
  for (const key of PILLAR_KEYS) pillars[key] = toNumberOrNull(record[key]);
  return pillars;
}
