...
todo lo ya escrito sigue funcionando. El tema oscuro se activa con
`prefers-color-scheme` redefiniendo sólo los primitivos en `globals.css`. En la
landing el esquema no sigue al sistema: hero y pie fuerzan oscuro y la banda de
producto fuerza claro, cada una redefiniendo los mismos primitivos en
`.landing-band-dark` / `.landing-band-light`; `data-band-theme` sólo tintea el
chrome de página (`body`, líneas del `SiteFrame`) para que interpole al paginar.
...
| 2026-09-18 (revisa 09-19) | Tema oscuro por `prefers-color-scheme` redefiniendo sólo los primitivos; cada banda de la landing los redefine en `.landing-band-dark` / `-light` y `data-band-theme` sólo tintea el chrome                                                                                                                                                                                                            | Sin conmutador no hay estado que guardar; el 2×2 heredaba navy a media transición                                                                                                                                                                       |
...
| 2026-09-20 (revisa 09-19) | Gráfica de la landing por nivel de score: cuatro niveles en su color; tira y rejilla comparten `selectedId`, en un loop de 7,2 s                                                                                                                                                                                                                                                                       | Hover sobre la página no cambiaba el chart; la paleta de severidad se lee antes de abrir el producto                                                                                                                                                    |
...
| 2026-09-20 (revisa 09-19) | Pie de landing sin Condiciones ni Privacidad: sólo © y la firma de producto, sin hairline ni «11 variables»; `LandingBar` en la fila 1240 (`px-4 sm:px-8`); copy de Producto a padding de nav, título sobre el plus y gráfica a `h-full`                                                                                                                                                               | El filete repetía Método; pegar la barra al gutter no coincidía con el dither; `justify-center` dejaba el bloque flotando                                                                                                                               |
