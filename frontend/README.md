...
| `GET /api/pulse/companies/{id}`            | Empresa con `series[]` (por mes: `pulse`, `confidence`, `pillars`, `variables`, `contributions`, `cash_end`) y `forecast[]` (`horizon`, `target_month`, `pulse_pred`, `pulse_p10`, `pulse_p90`, `delta_raw`, `contributions`); la app conserva solo los horizontes +1..+6.                                                                       |
