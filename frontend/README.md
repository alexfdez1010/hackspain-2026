...
| `/company/[id]/recommendations` | Financing: "Si necesitas financiación" with the approved products one per row (amount, rate, fit, a "Solicitar propuesta" button that confirms the request in a dialog, and their argument folded into "Ver detalle"), then "Fuera de alcance hoy" with the rule that leaves each product out and the disclaimer, and "Más detalle" with the improvement plan, risk and inputs used, all folded. The free operational measure lives on the Action pages.                                   |
