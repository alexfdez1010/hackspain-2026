...
data/raw/xray/*.csv                       (8 tables, see data_dictionary.md)
 ...
 data/output/{scored_panel.parquet,oof.parquet,submission.csv,xray_export.json}
        ...
        └─ export.py -> JSON contract consumed by ../web
```
...
`data/output/web/` and mirrors it into `web/src/data/xray/`.
...
with [`../web`](../web): every payload the Next.js app renders is served here.
...
|---|---|---|
| `XRAY_DATA_DIR` | `ml-service/data` | Root of the artefact folder |
