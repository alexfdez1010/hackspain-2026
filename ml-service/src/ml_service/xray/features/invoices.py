...
def _clean(ds: Dataset) -> pl.DataFrame:
    ...
    sane = lambda c: pl.when(  # noqa: E731
    ...
    return inv.with_columns(
        ...
        pl.when(pl.col("amount") > 0).then(pl.lit("ar")).otherwise(pl.lit("ap")).alias("side"),
        sane("due_date").alias("due_date"),
        sane("payment_date").alias("payment_date"),
...
def issued_features(inv: pl.DataFrame) -> pl.DataFrame:
    ...
    return out.pivot(on="side", index=["company_id", "month"], values=["issued", "n_issued"])
...
def paid_delay_features(inv: pl.DataFrame) -> pl.DataFrame:
    ...
    paid = inv.filter(pl.col("payment_date").is_not_null()).with_columns(
        (pl.col("payment_date") - pl.col("due_date")).dt.total_days().clip(MIN_DELAY_DAYS, MAX_DELAY_DAYS).alias("delay"),
    ...
    return out.pivot(on="side", index=["company_id", "month"], values=["delay_days", "late_share", "paid"])
...
def overdue_features(inv: pl.DataFrame) -> pl.DataFrame:
    ...
    end_m = pl.coalesce(pl.col("payment_date").dt.truncate("1mo"), pl.lit(LAST_FULL_MONTH) + pl.duration(days=40))
    rng = inv.select(
        "company_id", "side", "value", due_m.alias("start"), end_m.dt.truncate("1mo").alias("stop")
    ).filter(pl.col("start") <= pl.lit(LAST_FULL_MONTH))
    rng = rng.with_columns(
        pl.int_ranges(
            (pl.col("start").dt.year() - FIRST_MONTH.year) * 12 + pl.col("start").dt.month() - FIRST_MONTH.month + 1,
            (pl.col("stop").dt.year() - FIRST_MONTH.year) * 12 + pl.col("stop").dt.month() - FIRST_MONTH.month,
        ).alias("idx")
    )
    # invoice is overdue at month-end of every month strictly before its payment month
    exploded = rng.explode("idx").filter(pl.col("idx") >= 1).filter(pl.col("idx") <= len(months))
    ...
    return out.pivot(on="side", index=["company_id", "month"], values=["overdue", "n_overdue"])
...
def concentration_features(inv: pl.DataFrame) -> pl.DataFrame:
    ...
    frames = [
        monthly.with_columns(pl.col("month").dt.offset_by(f"{k}mo")) for k in range(HHI_WINDOW_MONTHS)
    ]
    window = pl.concat(frames).group_by("company_id", "counterparty_id", "month").agg(pl.col("value").sum())
    return (
        window.group_by("company_id", "month")
        .agg(
            ((pl.col("value") / pl.col("value").sum()) ** 2).sum().alias("customer_concentration"),
...
def build_invoice_features(ds: Dataset) -> pl.DataFrame:
    ...
    if not ds.table("invoices").height:
        return pl.DataFrame(schema={"company_id": pl.String, "month": pl.Datetime("us")})
    inv = _clean(ds)
