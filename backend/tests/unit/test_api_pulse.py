...
def _app(tmp_path: Path) -> TestClient:
    ...
    (details / "C1.json").write_text(
        json.dumps({"company_id": "C1", "month": "2026-08", "variables": {}})
    )
    settings = Settings(
        data_dir=tmp_path,
...
def test_details_are_served_from_the_export(tmp_path: Path):
    from ml_service.api import routes_pulse

    client = _app(tmp_path)
    client.app.include_router(routes_pulse.router)
    payload = client.get("/api/pulse/companies/C1/details").json()
    assert payload["month"] == "2026-08"
    assert payload["variables"] == {}
    assert client.get("/api/pulse/companies/unknown/details").status_code == 404
