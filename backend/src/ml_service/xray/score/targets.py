...
STRESS_RULES = {
    ...
    "stress_n": ("ge", 1.0),
    "payables_overdue_share": ("gt", 0.75),
    "loc_utilization": ("gt", 0.95),
    "neg_balance_share": ("gt", 0.5),
}


def stress_flag() -> pl.Expr:
    """Point-in-time stress condition (0/1); a missing input reads as "no stress".

    Nulls must be neutralised term by term: three-valued logic would turn the
    whole ``any_horizontal`` into a null whenever one input is missing (for
    example ``loc_utilization`` for a company without credit lines).
    """
    terms = [
        {"lt": pl.col(c) < t, "gt": pl.col(c) > t, "ge": pl.col(c) >= t}[op].fill_null(
            False
        )
        for c, (op, t) in STRESS_RULES.items()
    ]
    return pl.any_horizontal(terms).cast(pl.Int32)
...
def _future_any(col: str, op: str, thr: float, horizon: int) -> pl.Expr:
    ...
    shifted = [
        cond.shift(-k).over("company_id").fill_null(False)
...
def add_targets(panel: pl.DataFrame, horizon: int = HORIZON) -> pl.DataFrame:
    ...
    stress_terms = [
        _future_any(c, op, thr, horizon) for c, (op, thr) in STRESS_RULES.items()
    ]
    # Count how many of the next `horizon` months are stressful -> persistent stress.
    monthly_stress = stress_flag()
    n_stress = pl.sum_horizontal(
        [
            monthly_stress.shift(-k).over("company_id").fill_null(0)
            for k in range(1, horizon + 1)
        ]
    )
